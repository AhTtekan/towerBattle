...
public class BattleQueueGUI : MonoBehaviour
{
    ...
    private Transform Canvas
    {
        ...
        {
            if (canvas == null)
                canvas = transform.Find("QueueGUICanvas").Find("Placement");
            return canvas;
        }
    }
    ...
    public void Initialize(int length, float width = 0, float height = 0)
    {
        this.height = height;
        if (this.height == 0)
            this.height = Canvas.GetComponent<RectTransform>().rect.height;
        this.width = width;
        if (this.width == 0)
            this.width = Canvas.GetComponent<RectTransform>().rect.width;
